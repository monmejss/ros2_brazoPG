...
def main():
    ...
    # maneja solicitud de evaluación de un árbol genético
    ...
    f.close()
    # para que funcione lo del archivo gp
    for t in gp.population:
        t.setRosNode(None)
    if gp.bestParent:
        gp.bestParent.setRosNode(None)
    if gp.bestEver:
        gp.bestEver.setRosNode(None)
    ...
    # Guardar el mejor en Lisp
    if gp.bestEver:
        lisp_filename = "memoriaProcedural.lisp"
        with open(lisp_filename, "w") as f:
            f.write(gp.bestEver.toLisp())
        print("Archivo Lisp creado")
